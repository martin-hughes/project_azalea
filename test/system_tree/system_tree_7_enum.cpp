...
TEST(SystemTreeTest, SyscallEnums)
{
  ...
  ASSERT_EQ(syscall_open_handle("\\a", 2, &a_handle, 0), ERR_CODE::NO_ERROR);

  // Try to request an enum for a branch that doesn't exist.
  ASSERT_EQ(syscall_enum_children(a_handle + 1, nullptr, 0, 0, buf.get(), &variable_size), ERR_CODE::NOT_FOUND);

  // Feed a null buffer and see what size comes back.
  variable_size = 0;
  ASSERT_EQ(syscall_enum_children(a_handle, nullptr, 0, 0, nullptr, &variable_size), ERR_CODE::NO_ERROR);
  ...
  ASSERT_EQ(syscall_enum_children(a_handle, nullptr, 0, 0, buf.get(), &variable_size), ERR_CODE::NO_ERROR);
  ASSERT_EQ(variable_size, buf_size);
  ...
  variable_size = buf_size;
  ASSERT_EQ(syscall_enum_children(a_handle, nullptr, 0, 1, buf.get(), &variable_size), ERR_CODE::NO_ERROR);
  ...
  variable_size = buf_size;
  ASSERT_EQ(syscall_enum_children(a_handle, "branch_c", 8, 0, buf.get(), &variable_size), ERR_CODE::NO_ERROR);
