...
TEST_F(MemFsSyscallTests, CreateAndExit)
{
  ...
  ec = az_create_obj_and_handle(filename, strlen(filename), &new_file_handle);
  ASSERT_EQ(ec, ERR_CODE::NO_ERROR);
}
...
TEST_F(MemFsSyscallTests, CreateWriteAndRead)
{
  ...
  ec = az_write_handle(new_file_handle, 0, 23, test_string, 23, &br);
  ASSERT_EQ(ec, ERR_CODE::NO_ERROR);
  ASSERT_EQ(br, 23);

  br = 0;
  ec = az_get_handle_data_len(new_file_handle, &br);
  ASSERT_EQ(ec, ERR_CODE::NO_ERROR);
  ASSERT_EQ(br, 23);

  ec = az_close_handle(new_file_handle);
  ASSERT_EQ(ec, ERR_CODE::NO_ERROR);
  new_file_handle = 0;

  ec = az_open_handle(filename, strlen(filename), &new_file_handle, 0);
  ASSERT_EQ(ec, ERR_CODE::NO_ERROR);
  ...
  ec = az_read_handle(new_file_handle, 0, 23, output_buffer, 23, &br);
  ASSERT_EQ(ec, ERR_CODE::NO_ERROR);
  ASSERT_EQ(br, 23);
...
TEST_F(MemFsSyscallTests, FileDoesntExist)
{
  ...
  ec = az_open_handle(filename, strlen(filename), &fh, 0);
