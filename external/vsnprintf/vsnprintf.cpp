// This file contains a portable implementation of vsnprintf.cpp.
// It originates from ftp://ftp.jhweiss.de/pub/users/weiss/snprintf/snprintf.c, as of 2016-07-27.
...
// Change by Martin - don't include stdio.h
...
// Change by Martin
// Include "klib/misc/vargs.h" at the top of the file instead.
// #include <varargs.h>
// End of change

// Change by Martin - don't ignore last after all
#define VA_START(ap, last) va_start(ap, last) /* "last" is ignored. */
// End of change.
...
// Change by Martin - comment out all includes.
#if !HAVE_VSNPRINTF
  ...
  while (ch != '\0')
    ...
    case PRINT_S_CONV:
      ...
      case 'p':
         ...
         */
        // Change by Martin - Bring the assignment out of the if statement, and correct types.
        strvalue = (const char *)va_arg(args, void *);
        if (strvalue == (const char *)NULL)
        // End of change.
          /*
       ...
       int precision, int flags, int *overflow)
{
  ...
  UINTMAX_T fracpart;
  UINTMAX_T mask;
  // Change by Martin - explicit type conversion
  const char *infnan = (const char *)NULL;
  // End of change.
  char iconvert[MAX_CONVERT_LENGTH];
...
{
  // Change by Martin - add explicit type conversions
...
rpl_vasprintf(char **ret, const char *format, va_list ap)
{
  ...
  VA_COPY(aq, ap);
  //Change by Martin - explicit type conversion
  len = vsnprintf((char *)NULL, 0, format, aq);
  // End of change.
  VA_END_COPY(aq);
  // Change by Martin - explicit type conversion
...
#if !HAVE_SNPRINTF
// Change by Martin - We can use the argument list format.
//#if HAVE_STDARG_H
int
...
#if !HAVE_ASPRINTF
// Change by Martin - We can use the argument list declaration format.
//#if HAVE_STDARG_H
int
