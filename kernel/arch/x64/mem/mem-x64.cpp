...
#include "processor.h"
#include "../processor/processor-x64.h"
#include "mem.h"
...
namespace
{
  ...
  static ipc::raw_spinlock pml4_edit_lock;
...
void mem_gen_init(e820_pointer *e820_ptr)
{
  ...
  ipc_raw_spinlock_init(pml4_edit_lock);
                               ...
                               MEM_CACHE_MODES cache_mode)
{
  ...
  else
  {
    ...
    {
      ipc_raw_spinlock_lock(pml4_edit_lock);
    }
    ...
    *encoded_entry = mem_encode_page_table_entry(new_entry);
    ...
    if (is_kernel_allocation)
    {
      ...
      ipc_raw_spinlock_unlock(pml4_edit_lock);
    }
  }
