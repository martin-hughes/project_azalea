...
namespace
{
  ...
  ipc::raw_spinlock slabs_list_lock;
  ...
  char gen_lock_storage[sizeof(ipc::mutex)];
  ipc::mutex *allocator_gen_lock = nullptr;
...
void *kmalloc(uint64_t mem_size)
{
  ...
  KL_TRC_TRACE(TRC_LVL::FLOW, "Acquire lock\n");
  std::lock_guard<ipc::mutex> lock(*allocator_gen_lock);
  KL_TRC_TRACE(TRC_LVL::FLOW, "Lock acquired\n");
  ...
  ipc_raw_spinlock_lock(slabs_list_lock);
  if(!klib_list_is_empty(&partial_slabs_list[slab_idx]))
  {
    ...
    ipc_raw_spinlock_unlock(slabs_list_lock);
  }
...
void init_allocator_system()
{
  ...
  ipc_raw_spinlock_init(slabs_list_lock);
  allocator_gen_lock = reinterpret_cast<ipc::mutex *>(gen_lock_storage);
  allocator_gen_lock = new (allocator_gen_lock) ipc::mutex(true);
...
void test_only_reset_allocator()
{
  ...
  if (allocator_initialized)
  {
    ...
    allocator_gen_lock->~mutex();
    allocator_gen_lock = nullptr;
  }
